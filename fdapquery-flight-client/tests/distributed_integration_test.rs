...
use fdapquery_distributed::{DistributedConfig, ExecutorConfig, SessionContextExt};
...
async fn distributed_aggregate_query_end_to_end_via_flight() {
    ...
    let mut ctx = SessionContext::standalone(config.clone(), flight_client)
        .await
        .expect("SessionContext::standalone");
    ctx.register_csv("employee", EMPLOYEE_CSV);

    // Run the query. `SessionContext::sql` is sync (returns
    // `DataFrame`); `execute_data_frame` drives the plan through
    // the query planner and returns a stream. The scheduler
    // awaits every Flight call on the current tokio runtime; the
    // resulting `SendableRecordBatchStream` is drained via
    // `try_collect`.
    let df = ctx
