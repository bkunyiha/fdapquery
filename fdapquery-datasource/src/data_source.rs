...
//!
//! ## Notes
//! - `scan` returns a lazy `Box<dyn Iterator<Item = RecordBatch>>` of batches.
//!   Errors panic rather than being threaded through `Result<_>` — a future
//!   refactor will switch to `Stream<Item = Result<RecordBatch, FdapError>>`.
