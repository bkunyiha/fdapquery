...
async fn main() {
    ...
    // in-process scheduler, which then dispatches via Flight to
    // the (also in-process) executor bound above.
    let mut ctx = SessionContext::standalone(config, flight_client)
        .await
